...
def parse_key_value_pairs(kv_string: str | None) -> dict[str, str]:
    ...
    Silently skips any pair where the value is empty — the Agent Engine API
    rejects env vars with empty string values.
    """
    result = {}
    if kv_string:
        for pair in (p.strip() for p in kv_string.split(",") if p.strip()):
            if "=" in pair:
                ...
                if value:
                    ...
                    logging.warning(f"Skipping env var with empty value: {key}")
            else:
